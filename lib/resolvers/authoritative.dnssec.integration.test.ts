...
// over UDP/TCP:53. Gated behind `vitest run --mode network` (mapped to this
// env var in vitest.config.mts) so the default suite stays hermetic and the
// gate works on Windows shells too (no inline env vars).
